...
This guide explains how to consume the project's five deliverables in
production:

1. **`monitoring_use_cases`** — packaged Splunk app that hosts the compiled
   catalog inside Splunk chrome at `/en-US/app/monitoring_use_cases/catalog`.
2. **The web dashboard** (`index.html` / GitHub Pages) — browsable catalog
   for analysts outside Splunk.
3. **`TA-splunk-use-cases`** — Splunk Technology Add-on with Quick-Start
   scheduled searches.
4. **`DA-ITSI-monitoring-use-cases`** — ITSI content pack with KPI base
   searches, thresholds and service templates.
5. **`DA-ESS-monitoring-use-cases`** — Enterprise Security content pack
...
The packaged `monitoring_use_cases` app is the preferred in-platform delivery
model for the catalog itself: it keeps the overview, companion pages, API docs,
and data sizing tool inside standard Splunk chrome while preserving same-window
Search deep links.
...
## 2. Obtain the release and install the catalog app

Download the four `.spl` packages from the
[latest GitHub release](https://github.com/fenre/splunk-monitoring-use-cases/releases/latest):

```
monitoring_use_cases-<ver>.spl
...
All `.spl` archives generated by this repo are plain tar archives, so use
`tar -xf` when unpacking them on a deployer.

### 2.1 Install the `monitoring_use_cases` catalog app

**Splunk Enterprise (non-SHC):**

1. Log in to Splunk Web as an admin.
2. Navigate to *Apps → Manage Apps → Install app from file*.
3. Upload `monitoring_use_cases-<ver>.spl`.
4. Select **Upgrade app** when replacing an existing install.
5. Open *Monitoring Use Cases* from the app launcher, or navigate directly to
   `/en-US/app/monitoring_use_cases/catalog`.

**Search-head cluster (SHC):**

1. Copy the `.spl` onto the deployer:
   ```bash
   scp monitoring_use_cases-<ver>.spl deployer:/tmp/
   ```
2. Extract to the deployer's shcluster apps directory:
   ```bash
   tar -xf /tmp/monitoring_use_cases-<ver>.spl \
...
Upload `monitoring_use_cases-<ver>.spl` through the self-service installer or
ACS package workflow. The app is generator-owned, static-only, and has been
validated against AppInspect-relevant static checks.

### 2.2 Verify the hosted catalog

After install, verify these routes from inside Splunk:

- `/en-US/app/monitoring_use_cases/catalog` opens the compiled overview inside Splunk chrome.
- `/en-US/app/monitoring_use_cases/scorecard`, `/regulatory_primer`, `/clause_navigator`, `/compliance_story`, `/api_docs`, and `/data_sizing` each render in the same host model.
- `Open in Splunk Search` actions from hosted use-case detail panels stay in the current browser window and navigate into `/en-US/app/search/search?q=...`.
- The underlying static assets resolve under `/en-US/static/app/monitoring_use_cases/`.
...
2. Extract to the deployer's shcluster apps directory:
   ```bash
   tar -xf /tmp/TA-splunk-use-cases-<ver>.spl \
     -C $SPLUNK_HOME/etc/shcluster/apps/
   ```
3. Push the bundle:
   ```bash
...
The `monitoring_use_cases`, TA, ITSI, and ES packages are authored to pass:
...
| Dynamic code analysis | Splunk Cloud ACS pipeline | No custom server-side code in the catalog app, TA, ITSI, or ES packages; the catalog app ships static client-side assets only. |
...
2. **Enterprise internal static host** — copy the repo's root to any
   ...
   running `build.py`.
3. **Packaged Splunk app** — install `monitoring_use_cases-<ver>.spl` and open
   `/en-US/app/monitoring_use_cases/catalog`. The generator-owned host view keeps
   the catalog, companion pages, API docs, and the data sizing tool inside
   Splunk chrome without hand-maintained iframe XML.
...
tar -xf TA-splunk-use-cases-<previous>.spl \
...
- [ ] All four `.spl` files verified via SHA-256.
- [ ] `monitoring_use_cases` installed; `/en-US/app/monitoring_use_cases/catalog` and the planned companion routes load inside Splunk chrome.
- [ ] Same-window Search deep links verified from a hosted use-case detail panel.
